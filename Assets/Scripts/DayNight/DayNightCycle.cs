...
public class DayNightCycle : MonoBehaviour
{
    ...
    // This variable is a conversion between real time ie. 24 hours a day to game day time.
    ...
    [SerializeField]
    //Intensity at Sunrise and Sunset.
    private float sunBaseIntensity = 1f;
    //Change in Brightness from sunrise to noon and from noon to sunset.
    [SerializeField]
    private float sunVariation = 1.3f;
    [SerializeField]
    ...
    {
        //if ((_timeOfDay >= 0.3 && _timeOfDay <= 0.7) && lightsOn==false)
        if (_timeOfDay >= 0.25 && _timeOfDay <= 0.75)
        {
            //Debug.Log("Inside Night");
            //lightsOn = true;
            if (fireworksPlayed==false)
            {
        ...
        //else if((_timeOfDay < 0.3 || _timeOfDay > 0.7) && lightsOn == true)
        //else if (_timeOfDay < 0.3 || _timeOfDay > 0.7)
        else
        {
            //Debug.Log("Inside Day");
            //lightsOn = false;
